...
@section('meta_block')
    <meta name="description" lang="{{ app()->getLocale() }}"
          content="{!! $currentCategory == null ? $seo['seo_translation_name_categories_description'] : $seo['items_category_description'] !!}">
    <meta name="keywords" lang="{{ app()->getLocale() }}"
          content="{!! $currentCategory == null ? $seo['seo_translation_name_categories_keyword'] : $seo['items_category_keyword'] !!}">
    ...
    @if($currentCategory == null)
        <meta property="og:title"
              content="{!! $currentCategory == null ? $seo['seo_translation_name_categories_title'] : $seo['items_category_title'] !!}">
        ...
        <div class="container">
            <div class="sub_filter">
                @if($keywordParent !== null)
                    <a href="{{ route('items-list', $keywordParent) }}" class="active">
            ...
            <div class="naved_tit">
                    ...
                    <ol class="breadcrumb">
                        ...
                        @if($currentCategory != null)
                            <li class="breadcrumb-item"><a
                                    href="{{ route('items-list') }}">{{ trans('messages.Ads') }}</a></li>
                ...
                @include('items.item-header-list', ['active' => 2, 'keyword' => $keyword, 'address' => $address, 'urlParams' => $urlParams])
            </div>
            <div class="row">
                <div class="col-12">
                            ...
                            <div class="container">
                                @if($banners['item_list']['type'] == 1)
                                    <div class="phones_main"
                                         style="background-image: url({{ $banners['item_list']['img'] }});"
                                         alt="{{ $banners['item_list']['alt'] }}">
                                        <h2>{{ $banners['item_list']['title'] }}</h2>
                                        <a href="{{ route('banner-item', $banners['item_list']['id'] ) }}"
                                           rel="nofollow" class="more_know">{{ trans('messages.Details') }}</a>
                                    </div>
                                @else
                                    <div class="">
                                        <br>
                    ...
                    @endif
                    @if($currentCategory !== null)
                        <h1 class="title last_ret">{{ $seo['items_category_title_h1'] }}</h1>
                    @else
                        <h1 class="title last_ret">{{ $seo['seo_translation_name_categories_titleh1'] }}</h1>
                    @endif
                                ...
                                <div class="product_horizontal">
                                    ...
                                    @else
                                        <a href="{{ route('view-item', $item['link']) }}" class="product_item">
                                            ...
                                            @else
                                                <img class="lazy" src="{{ config('app.noImage') }}"
                                        ...
                                        @if($item['serviceFixed'])
                                            <div class="fastening"><img src="/images/fastening.png"
                                                                        alt="fastening">{{ trans('messages.Fix') }}
                                            </div>
                                        @endif
                                        @if($item['serviceQuick'])
                                            <div class="premium ups_pre"><img src="/images/premium.png"
                                                                              alt="premium">{{ trans('messages.Quick') }}
                                            </div>
                                        @endif
                                    @endif
                                </div>
                    ...
                    <noscript>
                        <div class="row col-12 mb-3">
                            <a class="btn-success__border mr-auto ml-auto"
                               href="{{ route('items-list', ['page' => $page, 'keyword' => $currentCategory['keyword'] ?? '']) }}">{{ trans('messages.More view') }}</a>
                        </div>
                    </noscript>
                </div>
            </div>
            ...
            <div class="container">
                ...
                <div class="rubrica_top_links">
                    @for($i = count($mainCategories) - 1; $i > -1; $i--)
                        <a href="{{ $keyword == $mainCategories[$i]['keyword'] ? 'javascript:void(0)' : route('items-list', $mainCategories[$i]['keyword']) }}">
        ...
        <div class="container">
            ...
            @foreach($itemInRegion as $region)
                <!-- <li><a href="#">{{ $region['name'] }} - <span>{{ $region['count'] }}</span></a></li> -->
                    <li>
                        <a href="{{ route('global-region', ['key' => $region['keyword'], 'time' => time()]) }}">{{ $region['name'] }}
                            - <span>{{ $region['count'] }}</a>
                    </li>
                @endforeach
            </ul>
