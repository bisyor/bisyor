        ...
        <div class="container">
            <nav aria-label="breadcrumb" class="my_nav">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ trans('messages.Ad Promotion') }}</li>
                </ol>
            </nav>
            @if($status == 'error')
            ...
            <div class="place_main">
                <a href="{{ route('view-item', $model->link) }}" class="insco">{{ trans('messages.View items') }}</a>
                <h1 class="mt-5">{{ trans('messages.Ad Promotion') }}</h1>
                <form action="{{ route('items-service-add', $model->id) }}" method="get" id="serivice-add" class="">
                    <div class="error"></div>
                    <div class="radio_mag innermag">
                        ...
                        <label for="notservice">
                            <img src="{{ config('app.noImage') }}" alt="square" style="max-width: 100px; max-height: 50px; margin: 5px">
                            {{ trans('messages.No promotion') }} <span style="position:absolute; right: 25px">{{ trans('messages.Free') }}</span>
                        </label>
                    ...
                    @foreach($services as $service)
                        <div class="radio_mag innermag">
                            <input type="radio" value="{{ $service->id }}" name="services" id="{{ $service->keyword }}" data-price="{{ number_format($service->price, 0, '.', ' ') }}">
                            <label for="{{ $service->keyword }}">
                                <img src="{{config('app.servicePath').$service->icon_s}}" alt="{{ $service->title }}" style="max-width: 100px; max-height: 100px; margin: 5px">
                                {{ $service->title }} <span style="position:absolute; right: 25px"><b>{{ number_format($service->price, 0, '.', ' ') }} </b>{{ trans('messages.sum') }}</span>
                            </label>
                    ...
                    <div class="radio_mag innermag">
                        ...
                        <label for="m1my"> 
                            ...
                            <b style="position: absolute; right: 5%; padding-left: 10px;">
                                {{ number_format(Auth::user()->balance, 0, '.', ' ') . " ".trans('messages.sum')}}
                            </b>
                        </label>
                    ...
                    <div class="btm_cl form-group text-center mt-5">
                        <a href="{{ route('profile-items-list') }}" class="cans">{{ trans('messages.Publish without ads') }}</a>
                        <button type="submit" class="op_ma">{{ trans('messages.Buy') }}</button>
                    </div>
                    <!-- <div class="form-group text-center mt-5">
                        <a href="{{ route('profile-items-list') }}" class="btn" style="color: #cdd5ff">{{ trans('messages.Publish without ads') }}</a>
