...
$this->title = $title;
$this->params['breadcrumbs'][] = ['label' => 'Услуги', 'url' => ['/items/services/index']];
...
<div class="panel panel-inverse">
    ...
    <div class="panel-body">
        ...
        <div class="shops-abonements-form" style="padding-right: 20px; padding-left: 20px;">
                    ...
                    <div id="<?=$lang->url?>" class="tab-pane fade <?=($i == 0) ? 'in active' : '' ?>">
                        <p>
                            <?php if($lang->url == 'ru'): ?>
                                    ...
                                    <?= $form->field($model, 'short_description',['template' => $templateInput])->textarea(['rows' => 7]) ?>
                            ...
                            <?php else: ?>
                                    <?= $form->field($model, 'translation_title['.$lang->url.']',['template' => $templateInput])->textInput(['value' => isset($titles[$lang->url]) ? $titles[$lang->url] : null ]) ?>
                                    <?= $form->field($model, 'translation_short_description['.$lang->url.']',['template' => $templateInput])->textarea(['rows' => 7]) ?>
                            ...
                            <div class="item row panel-bg-new">
                                <div class="col-md-11">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-4">
                                            <?= $form->field($modelRegionalPrice, "[{$i}]sections")->widget(Select2::classname(), [
            ...
            <?php if ($model->keyword == 'up'): ?>
                <?= $form->field($model, 'auto_enabled',[
                ...
                <?= $form->field($model, 'free_period',[
                    'template' => '<div class="row">
                            <div class="col-md-2">{label}</div>
                            <div class="col-md-1" >каждые</div>
                    ...
                    ])->textInput(['class' => 'number_input form-control','style' => 'height:25px !important;margin-bottom:2px;'])->hint('дней') ?>
            <?php endif ?>

            <?php if ($model->keyword == 'fix'): ?>
                ...
                <?php
                    $templateInput = '<div class="row" id="day_with_fix_keyword"><div class="col-md-2">
            ...
            <?php if ($model->keyword != 'press' && $model->keyword != 'fix' && $model->keyword != 'up'): ?>
                <?php
                    $templateInput = '<div class="row"><div class="col-md-2">
                        {label}</div><div class="col-md-1">{input}{error}</div><div class="col-md-4">{hint}</div></div>';
                ?>
                <?= $form->field($model, 'day',['template' => $templateInput])->textInput(['class' => 'number_input form-control','id' => 'services-day-2'])->hint('дней') ?>
            <?php endif ?>
            ...
            <?= $form->field($model, 'add_form',['template' => $templateCheckbox])->widget(CheckboxX::classname(),[
                    'pluginOptions'=>[
                            'threeState'=>false
                        ],
            ]) ?>
...
$this->registerJS(<<<JS
    ...
    changeDayType = function(){
        val = $("input:radio[name=\'Services[period_type]\']:checked").val();
        if(val == 1){
            $("#day_with_fix_keyword").show();
        }else{
            $("#services-day").val('1');
            $("#day_with_fix_keyword").hide();
        }
    }
