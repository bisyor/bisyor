...
$this->title = $title;
$this->params['breadcrumbs'][] = ['label' => 'Пакети услуг', 'url' => ['/items/packet/index']];
...
<div class="panel panel-inverse">
    ...
    <div class="panel-body">
        ...
        <div class="shops-abonements-form" style="padding-right: 20px; padding-left: 20px;">
                    ...
                    <div id="<?=$lang->url?>" class="tab-pane fade <?=($i == 0) ? 'in active' : '' ?>">
                        <p>
                            <?php if($lang->url == 'ru'): ?>
                                    ...
                                    <?= $form->field($model, 'short_description',['template' => $templateInput])->textarea(['rows' => 8]) ?>
                            ...
                            <?php else: ?>
                                    ...
                                    <?= $form->field($model, 'translation_short_description['.$lang->url.']',['template' => $templateInput])->textarea(['rows' => 8]) ?>
            ...
            <div class="row">
                <div class="col-md-2">
                    <label>Услуги входящие в пакет</label>
                </div>
                ...
                <?php foreach ($serviceModels as $i => $value): ?>
                    <?php
                        ...
                        $checkbox = $form->field($value, "[{$i}]status")->widget(CheckboxX::classname(), [
                            ...
                            'options' => [
                                ...
                                'onchange' => ' setValueCount($(this).val(),' . $id .')'
                            ]
                        ])->label(false);
                        ...
                        $templateInput = '<div class="row" style="margin-bottom: -15px;"><div class="col-md-3">
                            </div><div class="col-md-2">{input}{error}</div><div class="col-md-5">{hint}</div></div>';
                    ?>
                    <div class="row">
                    <div class="col-md-3">
                    <?=$checkbox?>
                    ...
                    <div class="col-md-9">
                        <?= $form->field($value, "[{$i}]value",['template' => $templateInput])->textInput(['class' => 'number_input form-control','style' => 'height:25px !important;margin-bottom:2px;','id' => 'serviceModel-value' . $id,'readonly' => ($value->value == 0) ? true : false])->hint($hint) ?>
                    </div>
                </div>
                <?php endforeach; ?>
...
$this->registerJS(<<<JS
    ...
    setValueCount = function(val,id){
        if(val != 1){
            $("#serviceModel-value" + id).prop('value','0');
            $("#serviceModel-value" + id).prop('readonly',true);
        }else{
            $("#serviceModel-value" + id).prop('readonly',false);
        }
    }
