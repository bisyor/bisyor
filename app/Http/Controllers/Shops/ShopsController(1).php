...
//        dd($card);
    ...
    public function update(Request $request)
    {
        $user = Auth::user();
        $additional = new Additional();
        $model = Shops::where(['keyword' => $request->keyword, 'user_id' => $user->id])->first();
        if (!$model) {
            abort(404);
        }
    ...
    public function saveUpdate(Request $request)
    {
        ...
        $shop->view_count = 0;
        if ($request->temp_address) {
            if($shop->logo && Storage::disk('ftp')->exists(config('app.shopsRoute') . $shop->logo)){
    ...
    /**
     * @param Request $request
     * Do'konga qoldirilgan izohlarni ko'rish uchun mahsus funksiya
     *
     * @return Application|\Illuminate\Contracts\View\Factory|\Illuminate\View\View|void
     */
    public function getShopComment(Request $request)
    {
        ...
        $shop = Shops::where(['keyword' => $keyword, 'status' => 1])
            ->with(['districts', 'sections', 'slides', 'ratings', 'subscribers', 'rating', 'comments'])->first();
        if ($shop == null) {
            return abort(404);
        }
        $segmants = Additional::getUrlSegmants(request()->segments());
